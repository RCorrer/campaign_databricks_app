...
function ruleRowHtml({ native, index, themes, nativeFields, rule = {} }) {
  ...
  const selectedTheme = themes.find(t => themeId(t) === rule.theme) || themes[0] || { fields: [] }
  const themeFields = selectedTheme.fields || []
  return `<div class="rule-row">
    <select class="select" name="theme-${index}" onchange="window.__campaignThemeChange && window.__campaignThemeChange('${index}', this.value)">${themes.map(t => `<option value="${themeId(t)}" ${rule.theme===themeId(t)?'selected':''}>${t.label}</option>`).join('')}</select>
...
async function segmentationPage(campaignId) {
  ...
  window.__campaignThemeChange = (index, themeCode) => {
    ...
    ruleMap.forEach(arr => arr.forEach(rule => {
      if (String(rule.__idx) === String(index)) {
        const theme = themes.find(t => themeId(t) === themeCode)
  ...
  document.getElementById('add-include').onclick = () => { groupsState.include.push({ theme: themeId(themes[0]) || 'cartoes', field: themes[0]?.fields?.[0]?.field || '', operator: 'EQUALS', value: '' }); rerenderRules() }
  document.getElementById('add-exclude').onclick = () => { groupsState.exclude.push({ theme: themeId(themes[0]) || 'cartoes', field: themes[0]?.fields?.[0]?.field || '', operator: 'EQUALS', value: '' }); rerenderRules() }
  ...
  function serializeRules(containerId, native) {
    ...
    if (native) {
      return [{ name: 'Grupo nativo', conditions }]
    }
    return conditions.map((condition, index) => ({
      name: `Grupo tematico ${index + 1}`,
        ...
        <form id="activation-form" class="form-grid">
          ...
          <div><label>Modo de execução</label>
            <select class="select" name="execution_mode">
              ${['RUN'].map(v => `<option value="${v}" ${activation.execution_mode===v?'selected':''}>${v}</option>`).join('')}
            </select>
          </div>
