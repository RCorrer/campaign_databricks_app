...
INSERT INTO main.customer_base.customer_master (
  cpf_cnpj, customer_name, tipo_cliente, segmento, subsegmento, idade, sexo, estado_civil,
  cidade, uf, cep, renda_mensal, faixa_renda, patrimonio_estimado, data_relacionamento_inicio,
  cluster_relacionamento, is_prime, is_exclusive, is_pj, is_varejo, is_jovem_digital, created_at
)
SELECT
  ...
  CAST(18 + (id % 52) AS INT) as idade,
  CASE WHEN id % 2 = 0 THEN 'F' ELSE 'M' END as sexo,
  CASE WHEN id % 4 = 0 THEN 'CASADO' ELSE 'SOLTEIRO' END as estado_civil,
  CASE
    WHEN id % 6 = 0 THEN 'Curitiba'
    WHEN id % 6 = 1 THEN 'São Paulo'
    WHEN id % 6 = 2 THEN 'Rio de Janeiro'
    WHEN id % 6 = 3 THEN 'Porto Alegre'
    WHEN id % 6 = 4 THEN 'Belo Horizonte'
    ELSE 'Florianópolis'
  END as cidade,
  CASE
    WHEN id % 6 IN (0, 5) THEN 'PR'
    WHEN id % 6 = 1 THEN 'SP'
    WHEN id % 6 = 2 THEN 'RJ'
    WHEN id % 6 = 3 THEN 'RS'
    ELSE 'MG'
  ...
  CASE
    WHEN (1500 + (id * 137 % 60000)) >= 25000 THEN 'ALTA'
    WHEN (1500 + (id * 137 % 60000)) >= 10000 THEN 'MEDIA'
    ELSE 'BAIXA'
  END as faixa_renda,
...
INSERT INTO main.customer_360.tb_contas (cpf_cnpj, conta_id, tipo_conta, status_conta, agencia, created_at)
SELECT
  cpf_cnpj,
  concat('CONTA-', cpf_cnpj),
  CASE WHEN tipo_cliente = 'PJ' THEN 'CORRENTE_PJ' ELSE 'CORRENTE' END,
  'ATIVA',
  concat('AG', substr(cpf_cnpj, 1, 4)),
  current_timestamp()
FROM main.customer_base.customer_master;

INSERT INTO main.customer_360.tb_saldos_conta (cpf_cnpj, conta_id, saldo_atual, saldo_medio_90d, created_at)
SELECT
  cpf_cnpj,
  concat('CONTA-', cpf_cnpj),
  CAST(renda_mensal * 2 AS DECIMAL(18,2)),
  CAST(renda_mensal * 1.5 AS DECIMAL(18,2)),
  current_timestamp()
FROM main.customer_base.customer_master;

INSERT INTO main.customer_360.tb_cartoes (cpf_cnpj, cartao_id, produto_cartao, bandeira, status_cartao, limite_total, created_at)
SELECT
  cpf_cnpj,
...
INSERT INTO main.customer_360.tb_gastos_cartao (cpf_cnpj, cartao_id, gasto_medio_90d, created_at)
SELECT
  cpf_cnpj,
  concat('CARTAO-', cpf_cnpj),
  CAST(renda_mensal * 0.35 AS DECIMAL(18,2)),
  current_timestamp()
FROM main.customer_base.customer_master;

INSERT INTO main.customer_360.tb_investimentos (cpf_cnpj, perfil_investidor, valor_total_investido, created_at)
SELECT
  cpf_cnpj,
...
INSERT INTO main.customer_360.tb_elegibilidade_ofertas (cpf_cnpj, oferta, elegivel, created_at)
...
INSERT INTO main.customer_360.tb_credito_perfil (cpf_cnpj, score_credito, atraso_max_dias, limite_pre_aprovado, created_at)
SELECT
  cpf_cnpj,
  CAST(400 + cast(substr(cpf_cnpj, 10, 2) as int) * 5 AS INT),
  CAST(cast(substr(cpf_cnpj, 9, 1) as int) * 3 AS INT),
  CAST(renda_mensal * 4 AS DECIMAL(18,2)),
  current_timestamp()
FROM main.customer_base.customer_master;

INSERT INTO main.customer_360.tb_emprestimos (cpf_cnpj, emprestimo_id, status_emprestimo, saldo_devedor, created_at)
SELECT
  cpf_cnpj,
  concat('EMP-', cpf_cnpj),
  'ATIVO',
  CAST(renda_mensal * 6 AS DECIMAL(18,2)),
  current_timestamp()
FROM main.customer_base.customer_master
WHERE cast(substr(cpf_cnpj, 11, 1) as int) % 3 = 0;

INSERT INTO main.customer_360.tb_canais_digitais (cpf_cnpj, canal_preferido, ultimo_acesso_app, created_at)
SELECT
  cpf_cnpj,
  CASE WHEN is_jovem_digital THEN 'APP' ELSE 'INTERNET_BANKING' END,
  date_sub(current_date(), CAST(cast(substr(cpf_cnpj, 11, 1) as int) AS INT)),
  current_timestamp()
FROM main.customer_base.customer_master;

INSERT INTO main.customer_360.tb_seguro_produtos (cpf_cnpj, produto_seguro, status_seguro, created_at)
SELECT
  cpf_cnpj,
  CASE WHEN is_pj THEN 'SEGURO_EMPRESARIAL' ELSE 'SEGURO_RESIDENCIAL' END,
  'ATIVO',
  current_timestamp()
FROM main.customer_base.customer_master
