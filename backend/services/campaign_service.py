...
VALUES (
  {sql_literal(campaign_id)}, 'UPDATE_CAMPAIGN', {sql_literal(json.dumps(req.model_dump()))}, current_timestamp(), 'app'
)
""")
...
WHERE campaign_id = {sql_literal(campaign_id)} AND is_current = TRUE
""")
        self.repo.execute(f"""
...
VALUES (
  {sql_literal(campaign_id)}, 'SAVE_BRIEFING', {sql_literal(json.dumps(req.model_dump()))}, current_timestamp(), 'app'
)
""")
...
""")
        self.repo.execute(f"""
UPDATE main.campaign_app.campaign_header
SET updated_at = current_timestamp(), updated_by = 'app'
WHERE campaign_id = {sql_literal(campaign_id)}
""")
...
""")
    ...
    def change_status(self, campaign_id: str, req: StatusChangeRequest) -> dict:
        current_rows = self.repo.execute(
            f"SELECT status FROM main.campaign_app.campaign_header WHERE campaign_id = {sql_literal(campaign_id)}"
        )
        if not current_rows:
            raise ValueError("Campanha não encontrada")
...
VALUES (
  {sql_literal(campaign_id)}, {sql_literal(current_status)}, {sql_literal(req.new_status)},
  {sql_literal(req.reason)}, current_timestamp(), 'app'
)
""")
